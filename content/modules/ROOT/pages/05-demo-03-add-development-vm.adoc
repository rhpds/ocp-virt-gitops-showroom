...
* *Argo CD Application*: `workshop-gitops-vms-dev`
...
. Switch to the `vms-dev` branch if needed:
...
. Configure Git author identity if not set yet (required once per terminal):
...
*Expected Result*: You should be on the `vms-dev` branch with the latest changes
